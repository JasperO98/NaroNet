...
import warnings
warnings.filterwarnings("ignore", message="`np.int` is a deprecated alias for the builtin `int`.*", category=DeprecationWarning)
warnings.filterwarnings("ignore", message="Passing \(type, 1\) or '1type' as a synonym of type is deprecated; in a future version of numpy, it will be understood as \(type, \(1,\)\) / '\(1,\)type'.", category=FutureWarning)
warnings.filterwarnings("ignore", message="`np.float` is a deprecated alias for the builtin `float`. To silence this warning, use `float` by itself.", category=DeprecationWarning)

import argparse
from datasets import load_mnist, load_reuters, load_usps, load_pendigits, load_embs
import csv, os
...
class IDEC(object):
    ...
    def pretrain(self, x, batch_size=256, epochs=10, optimizer='adam'):
        print('...Pretraining...')
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140,ae_weights=None, save_dir='./results/idec'):
        ...
        if not self.pretrained and ae_weights is None:
            ...
            print('   optimizer=\'adam\';   epochs=10')
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                if y is not None:
                    ...
                    print('Iter-%d: ACC= %.4f, NMI= %.4f, ARI= %.4f;  L= %.5f, Lc= %.5f,  Lr= %.5f' % (ite, acc, nmi, ari, loss[0], loss[1], loss[2]))
        ...
        # Get the reconstruction from the trained autoencoder
        reconstructed_x = self.autoencoder.predict(x)

        print('Shape of reconstructed_x:', reconstructed_x.shape)
        #print('Shape of clusters:', cls.shape)

        # Save the reconstruction to a .npy file
        np.save(save_dir + '/reconstructed_x.npy', reconstructed_x)
        print('Reconstruction saved to: ' + save_dir + '/reconstructed_x.npy')
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', default='mnist', choices=['mnist', 'usps', 'reutersidf10k', 'pendigits','embs'])
    ...
    if args.dataset == 'mnist':  # recommends: n_clusters=10, update_interval=140
        x, y = load_mnist()
    elif args.dataset == 'embs':  # recommends: n_clusters=10, update_interval=140
        x, y = load_embs()
    ...
    # Define IDEC model
    idec = IDEC(dims=[x.shape[-1], 150, 100, 10], n_clusters=args.n_clusters)
    #idec = IDEC(dims=[x.shape[-1], 128, 10], n_clusters=args.n_clusters)
    ...
    idec.fit(x, y=y, batch_size=args.batch_size, tol=args.tol, maxiter=args.maxiter, update_interval=args.update_interval, ae_weights=args.ae_weights, save_dir=args.save_dir)

    # Show the final results
    y_pred = idec.y_pred
    np.save('ypred2.npy', y_pred)
    if y is not None:
